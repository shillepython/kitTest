<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Switch kit-group on web-site.') }}
    </x-slot>
    ...
    <x-slot name="content">
        ...
        <div class="m-5">
            <div class="flex flex-wrap items-center">
                <span class="text-sm">Ваши группы:</span>
                @php
                    foreach (\App\Models\Group::all()->where('user_id', Auth::user()->getAuthIdentifier()) as $group){
                        if (!$group->verified == false){
                        @endphp
                            <span class="px-2 py-0 bg-green-200 text-sm text-black mx-1 rounded border-green-600">{{ $group->group->name }} (подтверждено)</span>
                        @php
                        }else{
                        @endphp
                            <span class="px-2 py-0 bg-red-200 text-sm text-white mx-1 rounded border-red-400">{{ $group->group->name }} (не подтверждено)</span>
                        @php
                        }
                    }
                @endphp
            </div>
            <select x-cloak id="group">
            @foreach (\App\Models\GroupId::all() as $groups)
                    <option value="{{ $groups->id }}">{{ $groups->name }}</option>
            @endforeach
            </select>
                <div x-data="dropdown()" x-init="loadOptions()" class="w-full">
                    <input name="group" type="hidden" x-bind:value="selectedValues()">
        ...
        " />
            ...
            <div class="flex items-center justify-start px-4 text-left sm:px-6">
                <x-jet-button>
                    {{ __('Save') }}
                </x-jet-button>
            </div>
