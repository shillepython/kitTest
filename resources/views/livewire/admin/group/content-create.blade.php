    ...
    <div class="flex flex-wrap">
        <div class="w-full px-4">
            <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-gray-50 border-0">
                ...
                <form action="{{ route('store-new-group', app()->getLocale()) }}" method="post">
                    @csrf
                    <div class="rounded-t bg-white mb-0 px-6 py-6">
                        <div class="text-center flex justify-between">
                            <h6 class="text-gray-800 text-xl font-bold">{{ __('Created') }}</h6>
                        ...
                        <div class="flex flex-wrap">
                            <div class="w-full px-4">
                                <div class="relative w-full mb-3">
                                    ...
                                    <x-jet-input id="name" name="name" type="text" class="w-full transition-all duration-150" value="{{ old('name') }}"/>
                                </div>
                            </div>
                            <div class="w-full px-4">
                                <div class="relative w-full mb-3">
                                    <x-jet-label for="desc" value="{{ __('Description group') }}" />
                                    <textarea type="text" class="form-textarea mt-1 block w-full rounded-md shadow-sm transition duration-300 ease-out transform border-gray-300" rows="3" placeholder="{{ __('Enter some long group description.') }}" name="desc" id="desc">{{ old('desc') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
